...
private:
	int camPares(Link raiz, int& M) {
		if (raiz == nullptr) { return 0; }
		if (raiz->elem % 2 == 1) {
			int M1 = 0, M2 = 0;
			int maxiz = camPares(raiz->left, M1), maxdr = camPares(raiz->right, M2);
			if (maxiz < maxdr) { maxiz = maxdr; }
			M = 0;
			return maxiz;
		}
		else{
			int M1 = 0, M2 = 0;
			int max = 1, maxiz = camPares(raiz->left, M1), maxdr = camPares(raiz->right, M2);
			if (M1 > 0) { max += M1; }
			if (M2 > 0) { max += M2; }
			if (M1 < M2) { M1 = M2; }
			M = M1 + 1;
			if (max < maxiz) { max = maxiz; }
			if (max < maxdr) { max = maxdr; }
			return max;
		}
	}
