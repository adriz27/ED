...
public:
	...
	int count_interval(T const& lower, T const& upper) const {
		int n = 0;
		if (!this->empty()) { ci(lower, upper, n, raiz); }
		return n;
	}

protected:

	void ci(T const& l, T const& u, int& n, Link a) const {
		if (menor(u, a->elem)) { if (a->iz != nullptr) { ci(l, u, n, a->iz); } }
		else if (menor(a->elem, l)) { if (a->dr != nullptr) { ci(l, u, n, a->dr); } }
		else {
			n++;
			if (a->iz != nullptr && a->elem != l) { ci(l, u, n, a->iz); }
			if (a->dr != nullptr && a->elem != u) { ci(l, u, n, a->dr); }
		}
	}

	void lower(T const& e, Link a, std::pair<bool, T>& p) {
		if (a == nullptr) { return;}
		if (menor(a->elem, e)) {
			lower(e, a->dr, p);
		}
